...
const PROFILE_DIR = path.join(os.homedir(), '.google_meet_agent', 'chrome-profile');
...
async function clickIfPresent(page: Page, selector: string, timeout = 2000): Promise<boolean> {
  try {
...
async function readDevtoolsEndpoint(): Promise<string | null> {
  try {
    const raw = fs.readFileSync(path.join(PROFILE_DIR, 'DevToolsActivePort'), 'utf8').trim();
...
async function fillGuestName(page: Page, name: string): Promise<void> {
  ...
  await page.evaluate(() => {
    const el = document.querySelector('input[aria-label="Your name"]') as HTMLInputElement | null;
    if (!el) return;
...
async function setMic(page: Page, wantMuted: boolean): Promise<void> {
  const attrBtn = await page.$('div[role="button"][data-is-muted], button[data-is-muted]');
  if (attrBtn) {
 ...
 */
export async function joinMeet(meetUrl: string, opts: JoinOptions = {}): Promise<Page> {
  ...
  const page: Page =
    meetPage ||
    (pages[0] && pages[0].url() === 'about:blank' ? pages[0] : await browser.newPage());
  page.setDefaultTimeout(PREJOIN_TIMEOUT_MS);

  if (await isInCall(page)) {
    console.log('Already in the meeting — skipping prejoin');
    return page;
  }
